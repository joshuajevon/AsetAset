...
@section('body')
    <div class="flex">
        ...
        <div
            ...
            <h1 class="text-2xl lg:text-3xl xl:text-4xl 2xl:text-5xl font-bold text-cGold">Add Slideshow</h1>
            ...
            <form
                ...
                action="{{ route('store-carousel') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="w-full">
                    <x-input-label for="slideshow-title" :value="__('Judul')" />
                    <x-text-input type="text" id="slideshow-title" class="mt-1 w-full" placeholder="Masukkan judul slideshow" name="title"
                        value="{{ old('title') }}" />
                ...
                <div class="w-full">
                    ...
                    <input type="file" class="mt-1 w-full form-control @error('slideshow') is-invalid @enderror" id="slideshow-image"
                        name="slideshow" value="{{ old('slideshow') }}">
                    <x-input-error :messages="$errors->get('slideshow')" class="mt-2" />
                </div>

                <div class="w-full">
                    <x-input-label for="slideshow-link" :value="__('Link (Opsional)')" />
                    <x-text-input type="text" id="slideshow-link" class="mt-1 w-full" placeholder="Masukkan link slideshow" name="link"
                        value="{{ old('link') }}" />
